#!/usr/bin/env python3
"""Segment videos using diarization JSON and emit Kaldi data dir.
...
One JSON per recording, named with the same base name as the video file
(e.g., foo.mov -> foo.json).
"""
...
def find_video_for_recording(video_dir: Path, recording_id: str) -> Path | None:
    for ext in VIDEO_EXTS:
        candidate = video_dir / f"{recording_id}{ext}"
        if candidate.exists():
            return candidate
    return None
